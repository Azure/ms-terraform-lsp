...
func parseSnippet(filepath string) (*Snippet, error) {
	...
	addrUrlMap := make(map[string]string)
	for _, block := range blocks {
		if block.Type == "resource" || block.Type == "data" {
			...
			addrUrlMap[addr] = urlValue
		}
	}

	lastBlock := blocks[len(blocks)-1]
	if lastBlock.Labels[0] != "msgraph_resource" {
		// skip non msgraph_resource block
		return nil, nil
	}
	urlValue := ""
	fields := make([]Field, 0)
	...
	for _, attr := range attributes {
		...
		for _, variable := range vars {
			...
			placeholder := placeholderContent(rawContent, addrUrlMap)
			format := `${%d:"%s"}`
