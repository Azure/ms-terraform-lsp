...
func Test_SnippetGen(t *testing.T) {
	exampleDir := os.Getenv("MSGRAPH_EXAMPLE_DIR")
	if exampleDir == "" {
		t.Skip("MSGRAPH_EXAMPLE_DIR is not set")
	}
	...
	slices.SortFunc(snippets, func(i, j Snippet) int {
		return strings.Compare(i.ResourceType, j.ResourceType)
	})
...
%s
	...
	for _, snippet := range snippets {
		lowerResourceType := strings.ToLower(snippet.ResourceType)
...
`, lowerResourceType, snippet.ResourceType, fields)
	}
...
func Test_parseSnippet(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		if actual.ResourceType != testcase.expected.ResourceType {
			t.Errorf("expected %s, but got %s", testcase.expected.ResourceType, actual.ResourceType)
		}
...
func Test_PlaceholderContent(t *testing.T) {
	testcases := []struct {
		...
		{
			rawContent:  "msgraph_resource.rg.output",
			addrTypeMap: map[string]string{},
			expected:    "TODO",
		},
		{
			rawContent: "data.msgraph_resource.rg.id",
			addrTypeMap: map[string]string{
				"data.msgraph_resource.rg": "applications",
			},
			expected: "The id of the applications resource",
		},
		{
			rawContent: "msgraph_resource.rg.id",
			addrTypeMap: map[string]string{
				"msgraph_resource.rg": "applications",
			},
			expected: "The id of the applications resource",
		},
		{
			rawContent: "data.msgraph_resource.rg.foo",
			addrTypeMap: map[string]string{
				"data.msgraph_resource.rg": "applications",
			},
			expected: "The foo of the applications resource",
		},
		{
			rawContent: "msgraph_resource.rg.foo",
			addrTypeMap: map[string]string{
				"msgraph_resource.rg": "applications",
			},
			expected: "The foo of the applications resource",
